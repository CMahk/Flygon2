				...
				if (link.get('href')[-1 * len(extension):] == extension):
					...
					async with session.get('https://play.pokemonshowdown.com/sprites/' + url + '/' + link.get('href')) as fileResp:
						if fileResp.status == 200:
							image = await aiofiles.open(PATH + '\\sprite\\' + url + '\\' + link.get('href'), mode = 'wb')
...
def makeFolders(list):
	if not os.path.exists(PATH + '\\sprite'):
		os.mkdir(PATH + '\\sprite')

	for name in list:
		if not os.path.exists(PATH + '\\sprite\\' + name):
			os.mkdir(PATH + '\\sprite\\' + name)
