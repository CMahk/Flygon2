...
path = os.path.abspath(os.path.dirname(__file__) + os.sep + os.pardir)
...
listMega = loadList('sql\\mega.txt')
listAlolan = loadList('sql\\alolan.txt')
listGalarian = loadList('sql\\galarian.txt')
listGmax = loadList('sql\\gmax.txt')

index = 1
with open(path + '\\sql\\species.txt', 'r') as infile:
	for line in infile:
		line = line.rstrip()
...
with open(path + '\\sql\\forme.txt', 'r') as infile:
	for line in infile:
		line = line.rstrip()
		line = re.sub('[:,]', '', line)
		split = line.split(' ')
